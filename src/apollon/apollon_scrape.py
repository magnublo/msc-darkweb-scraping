...
class ApollonScrapingSession(BaseScraper):
    ...
    def _scrape_seller(self, seller_url, seller, is_new_seller):
        ...
        most_recent_feedbacks_from_storage = self.db_session.query(Feedback).filter(Feedback.seller_id == 5,
        ...
        most_recent_feedbacks_from_storage: Tuple[Feedback]
        most_recent_feedback_texts_from_storage = [f.feedback_message_text for f in most_recent_feedbacks_from_storage]
        if most_recent_feedback_text not in most_recent_feedback_texts_from_storage:
            for category, feedback_page_url in zip(feedback_categories, feedback_urls):
                self._scrape_feedback(seller, is_new_seller, category, feedback_page_url)
